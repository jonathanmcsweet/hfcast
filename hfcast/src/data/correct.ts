 ...
 *
 * The same shape the listing parser produces, declared here rather than
 * imported from it: the application calls the engine directly and never parses
 * a printed listing, so it would otherwise import a parser it cannot use.
 */
export interface RawBandHour {
  hour: number;
  band: BandKey;
  reliability: number;
  snr: number;
