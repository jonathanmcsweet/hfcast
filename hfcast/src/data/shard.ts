 ...
 * below is for.
 *
 * A copy of `server/src/voacap/shard.ts`, pinned character for character
 * from the marker below by `server/test/shared-with-app.test.ts`. The
 * app cuts its own grid the same way when it runs the fine globe on the
 * device, and two sets of this arithmetic would put the two paths on
 * different lattices — the same map drawn from cells half a step apart,
